...
# CategorySale
def getCategorySale(yearvalue):
    conn = connect()
    CategorySale = pd.read_sql('call CategorySalesYear (' + yearvalue + ');', conn)
    return CategorySale
...
def categorysale(yearvalue):
    CategorySale = getCategorySale(yearvalue)
...
############################## Drop Down ########################
dropdown = dcc.Dropdown(['1996', '1997'], '1997')
############################ End Drop Down ######################

################################ Dash layout ##########################
# Inklusiv navnbar
app.layout = html.Div(id = 'parent', children = [topbar, dropdown, body_app])
############################## End ash layout ########################
